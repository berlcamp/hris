...
// the grace week opens a THIRD month on top of the usual two, and today is
// always the ceiling.
...
test("two payroll months are open: this one and the one before it", () => {
  assert.equal(CORRECTION_OPEN_MONTHS, 2);
...
test("day 7 is the last day of grace; day 8 closes the oldest month", () => {
  ...
  assert.equal(correctionWindow("2026-08-07").from, "2026-06-01");
  assert.equal(correctionWindow("2026-08-08").from, "2026-07-01");
});

test("the window rolls over the year", () => {
  // January 3 is inside the grace week, so the window reaches back to November.
  assert.deepEqual(correctionWindow("2027-01-03"), {
    from: "2026-11-01",
    to: "2027-01-03",
  });
  // January 9 is past it — December of the previous year is the oldest open one.
  assert.deepEqual(correctionWindow("2027-01-09"), {
    from: "2026-12-01",
    to: "2027-01-09",
  });
  // February past the grace week: December closes, January stays.
...
test("membership covers every open month and rejects what is older", () => {
  const w = correctionWindow("2026-08-05"); // Jun 1 .. Aug 5
  assert.equal(isDateInCorrectionWindow("2026-06-01", w), true);
  assert.equal(isDateInCorrectionWindow("2026-07-31", w), true);
  assert.equal(isDateInCorrectionWindow("2026-08-05", w), true);
  assert.equal(isDateInCorrectionWindow("2026-05-31", w), false);

  assert.equal(correctionWindowError("2026-06-15", w), null);
  assert.match(
    correctionWindowError("2026-05-31", w) ?? "",
    /outside the payroll months/,
  );
});

test("a June correction filed on August 8 is refused", () => {
  // The grace week has closed, so June belongs to HR now — but July, the other
  // open month, is still the department's to fix.
  const w = correctionWindow("2026-08-08");
  assert.equal(isDateInCorrectionWindow("2026-06-20", w), false);
...
test("the window describes itself for the filing form", () => {
  ...
  assert.equal(
    ...
    "November 1, 2026 – January 3, 2027",
  );
});
