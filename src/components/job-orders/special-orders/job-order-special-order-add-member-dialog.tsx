...
import {
  addJobOrderSpecialOrderMembers,
  getAddableJobOrdersForSpecialOrder,
} from "@/lib/actions/job-order-special-order-actions";
import type { JobOrderSpecialOrderPickerOption } from "@/lib/types";

interface JobOrderSpecialOrderAddMemberDialogProps {
  open: boolean;
...
export function JobOrderSpecialOrderAddMemberDialog({
  open,
  onOpenChange,
  specialOrderId,
  onAdded,
}: JobOrderSpecialOrderAddMemberDialogProps) {
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [search, setSearch] = useState("");
  const [options, setOptions] = useState<JobOrderSpecialOrderPickerOption[]>([]);
  ...
  useEffect(() => {
    ...
    setSelected([]);
    setLoading(true);
    getAddableJobOrdersForSpecialOrder(specialOrderId)
      .then(setOptions)
      .catch(() => {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const result = await addJobOrderSpecialOrderMembers(
        specialOrderId,
        selected,
      );
      if (result.error) {
      ...
      toast.success(
        `${result.data!.added} ${result.data!.added === 1 ? "person" : "people"} added.`,
      );
      onOpenChange(false);
      onAdded();
    } catch {
      toast.error("Something went wrong adding these people. Please try again.");
    } finally {
      setSaving(false);
    }
  };
