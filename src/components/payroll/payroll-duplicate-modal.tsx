...
interface Props {
  source: PayrollListRow | null;
  onOpenChange: (o: boolean) => void;
  onSuccess?: () => void;
}

function nextMonthRange(periodEnd: string): { start: string; end: string } {
...
export function PayrollDuplicateModal({ source, onOpenChange, onSuccess }: Props) {
  const open = !!source;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!source) return;
    setSaving(true);
    const res = await duplicatePayroll(source.id, {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Clones all employees and their deductions to a new period; net pay
            is recomputed.
          </DialogDescription>
        </DialogHeader>
